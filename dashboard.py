...
plt.subplots_adjust(bottom=0.25, top=0.8,wspace=0.4)
...
def run_simulation(_=None):
    run_count[0] += 1
    qber, status = run_with_eve(n=300, eve_present=eve_mode[0])
...
def update_plots(qber, status):
    ...
    # Plot 2: Network diagram (3 nodes)
    ax2 = axes[1]
    ax2.clear()
    ax2.set_xlim(0, 10)
    ax2.set_ylim(0, 5)
    ...
    nodes = [('Alice', 1.5, 2.5, '#185FA5'), ('Relay', 5, 2.5, '#1D9E75'), ('Bob', 8.5, 2.5, '#185FA5')]
    for name, x, y, c in nodes:
        ax2.add_patch(plt.Circle((x, y), 0.7, color=c, zorder=3))
        ax2.text(x, y, name[0], ha='center', va='center', color='white', fontweight='bold', fontsize=12, zorder=4)
    ...
    if eve_mode[0]:
        ...
        ax2.text(3.25, 3.8, 'E', ha='center', va='center', color='white', fontweight='bold', fontsize=12, zorder=4)
    ...
    # Plot 3: Key stats bar chart
    ax3 = axes[2]
    ax3.clear()
    n = 300
    alice_bits = generate_bits(n); alice_bases = generate_bases(n)
    ...
    bars = ax3.bar(['Qubits sent', 'Sifted key', 'Final key'],
    ...
    for bar in bars:
        h = bar.get_height()
        ax3.text(bar.get_x() + bar.get_width()/2, h + 3, str(int(h)), ha='center', fontsize=9)

    plt.draw()

# Buttons
ax_run = plt.axes([0.35, 0.08, 0.15, 0.06])
ax_eve = plt.axes([0.52, 0.08, 0.15, 0.06])
